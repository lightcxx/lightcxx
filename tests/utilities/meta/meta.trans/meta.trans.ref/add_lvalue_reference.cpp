...
constexpr bool test_add_lvalue_reference() {
    static_assert(std::is_same_v<std::add_lvalue_reference_t<T>, U>);
    static_assert(std::is_same_v<typename std::add_lvalue_reference<T>::type, U>);
    return true;
}
...
template<class T> requires is_void_v<T> || is_lvalue_reference_v<T>
...
static_assert(test_add_lvalue_reference<int() const, int() const>());
static_assert(test_add_lvalue_reference<int() volatile, int() volatile>());
static_assert(test_add_lvalue_reference<int() const volatile, int() const volatile>());

...
TEST() {
    ...
    } catch (...) { fail("Could not catch thrown std::bad_alloc by catching std::bad_alloc&"); }

    // TODO: Test that bad_alloc is polymorphic and extends std::exception (requires <type_traits>).
}

...
using _AtExitHandlerPtr = _Light::_AtExitHandler*;

atomic_size_t __num_quick_exit_handlers = 0;
_AtExitHandlerPtr __quick_exit_handlers[32]{};
...
int at_quick_exit(_Light::_AtExitHandler* func) noexcept {
    const auto index = atomic_fetch_add(&__num_quick_exit_handlers, 1);
    if (index >= 32) { return 1; }
    __quick_exit_handlers[index] = func;
    return 0;
}
